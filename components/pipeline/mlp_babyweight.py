...
def preprocess_train_and_deploy(
    ...
    preprocess = dsl.ContainerOp(
          ...
          image='gcr.io/tenacious-camp-267214/babyweight-pipeline-bqtocsv:latest',
          arguments=[
            '--project', project,
            '--mode', 'cloud',
            '--bucket', bucket,
    ...
    hparam_train = dsl.ContainerOp(
        name='hypertrain',
        # image needs to be a compile-time string
        image='gcr.io/tenacious-camp-267214/babyweight-pipeline-hypertrain:latest',
        arguments=[
...
def train_and_deploy_helper(preprocess, hparam_train):
    ...
    train_tuned = dsl.ContainerOp(
      name='traintuned',
      # image needs to be a compile-time string
      image='gcr.io/tenacious-camp-267214/babyweight-pipeline-traintuned:latest',
      arguments=[
